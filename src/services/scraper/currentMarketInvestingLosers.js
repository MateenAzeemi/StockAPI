...
async function scrapeCurrentMarketInvestingLosers() {
  try {
    const url = 'https://www.investing.com/equities/top-stock-losers';
    logger.info(`Scraping Investing Losers: ${url}`);
    ...
    logger.info(`Investing Losers: Found ${stocks.length} stocks`);
    return stocks;
  } catch (error) {
    logger.error('Error scraping Investing Losers:', error.message);
    return [];
  }
}

module.exports = scrapeCurrentMarketInvestingLosers;
