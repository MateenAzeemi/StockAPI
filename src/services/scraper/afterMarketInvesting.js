...
async function scrapeAfterMarketInvesting() {
  try {
    const url = 'https://www.investing.com/equities/after-hours';
    logger.info(`Scraping Investing After-Market: ${url}`);
    ...
    logger.info(`Investing After-Market: Found ${stocks.length} stocks`);
    return stocks;
  } catch (error) {
    logger.error('Error scraping Investing After-Market:', error.message);
    return [];
  }
}

module.exports = scrapeAfterMarketInvesting;
