...
async function scrapeCurrentMarketInvestingGainers() {
  try {
    const url = 'https://www.investing.com/equities/top-stock-gainers';
    logger.info(`Scraping Investing Gainers: ${url}`);
    ...
    logger.info(`Investing Gainers: Found ${stocks.length} stocks`);
    return stocks;
  } catch (error) {
    logger.error('Error scraping Investing Gainers:', error.message);
    return [];
  }
}

module.exports = scrapeCurrentMarketInvestingGainers;
